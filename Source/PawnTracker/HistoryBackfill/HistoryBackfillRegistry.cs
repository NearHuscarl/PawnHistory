...
internal static class HistoryBackfillRegistry
{
    ...
    {
        var definitions = new Dictionary<HistoryRecordDef, HistoryBackfillDefinition>();
        
        definitions.Add(HistoryRecordDefOf.BodyPartScarred, new HistoryBackfillDefinition(HistoryRecordDefOf.BodyPartScarred, "HealthPrehistory")
            .AddHard(
        ...
        definitions.Add(HistoryRecordDefOf.BodyPartDestroyed, new HistoryBackfillDefinition(HistoryRecordDefOf.BodyPartDestroyed, "HealthPrehistory")
            .AddHard(
                new MinimumAgeRule(7f),
        ...
        BuildRoyaltyDefinitions(definitions);
        BuildBiotechDefinitions(definitions);

        return definitions;
    }

    private static void BuildRoyaltyDefinitions(Dictionary<HistoryRecordDef, HistoryBackfillDefinition> definitions)
    {
        if (!ModsConfig.RoyaltyActive)
            return;
        
        definitions.Add(HistoryRecordDefOf.TitleGained, new HistoryBackfillDefinition(HistoryRecordDefOf.TitleGained)
            .AddHard(
    ...
    private static void BuildBiotechDefinitions(Dictionary<HistoryRecordDef, HistoryBackfillDefinition> definitions)
    {
        if (!ModsConfig.BiotechActive)
            return;
        
        definitions.TryAdd(HistoryRecordDefOf.MechlinkInstalled, new HistoryBackfillDefinition(HistoryRecordDefOf.MechlinkInstalled)
            .AddHard(
                new MinimumAgeRule(13f),
                new MaximumCountRule(1),
